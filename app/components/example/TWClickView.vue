<template>
  <div class="2xl:container 2xl:mx-auto md:py-12 lg:px-20 md:px-6 py-9 px-4">
    ...
    >
      ...
      <div
        class="mt-3 md:mt-4 lg:mt-0 flex flex-col lg:flex-row items-stretch justify-center lg:space-x-8"
      >
        <div
          class="lg:w-1/2 flex justify-between items-stretch bg-gray-50 px-2 py-20 md:py-6 md:px-6 lg:py-24"
        >
          ...
          <div class="slider">
            <div ref="slideAna" class="slide-ana lg:relative">
              <div class="flex" style="transform: translateX(-100%)">
                <img
                  src="/pay2me/alipay.png"
        ...
        >
          ...
          >
            <button
              class="w-full md:w-3/5 border border-gray-800 text-base font-medium leading-none uppercase py-6 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 bg-gray-800 text-white dark:bg-white dark:text-gray-900 dark:hover:bg-gray-200"
            >
              Add to Cart
            </button>
            <button
              class="w-full md:w-2/5 border border-gray-800 text-base font-medium leading-none text-gray-800 uppercase py-6 bg-transparent focus:outline-none focus:ring-2 focus:ring-offset-2 dark:bg-transparent dark:border-white dark:text-white focus:ring-gray-800 hover:bg-gray-800 hover:text-white dark:hover:bg-gray-800"
            >
              View Details
            </button>
          </div>
          <div class="mt-6">
            <button
              class="text-xl underline text-gray-800 dark:text-white dark:hover:text-gray-200 capitalize hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800"
            >
              add to wishlist
            </button>
          </div>
...
const viewerButton = ref<HTMLElement | null>(null);
const viewerBox = ref<HTMLElement | null>(null);
const slideAna = ref<HTMLElement | null>(null);

const showViewerButton = ref(true);
const showViewerBox = ref(false);
...
function updateTransforms() {
  ...
  slideElements.forEach((element, index) => {
    element.style.transform = `translateX(${100 * (index - (loop % slideElements.length))}%)`;
  });
}
...
function openView() {
  showViewerButton.value = false;
  showViewerBox.value = true;
}

function closeView() {
  showViewerBox.value = false;
  showViewerButton.value = true;
}

onMounted(() => {
  if (slideAna.value) {
    const nodes = Array.from(
      slideAna.value.querySelectorAll(":scope > div")
    ) as HTMLElement[];
    slideElements = nodes;
    slideElements.forEach((element, index) => {
      element.style.transform = `translateX(${100 * index}%)`;
    });
    loop = 0 + 1000 * slideElements.length;
  }
});
</script>
